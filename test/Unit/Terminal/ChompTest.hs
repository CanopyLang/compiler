  ...
  testGroup
    ...
    [ testCase "createSuggest with valid index" $ do
        ...
        case suggest of
          ...
          _ -> assertFailure "Expected SuggestAt",
...
testErrorHandling =
  testGroup
    ...
    [ testCase "invalid chunk index handling" $ do
        ...
        case result of
          Left _ -> pure () -- Expected error for zero index
          Right _ -> assertFailure "Should fail with zero index",
      testCase "error propagation in chomp" $ do
        ...
        case result of
          Left _ -> pure () -- Expected parsing error
          Right _ -> assertFailure "Should fail with invalid args"
    ]
