...
tests :: TestTree
tests = Test.testGroup "Init.Project Tests"
  [ outlineConfigTests
  , directorySetupTests
...
outlineConfigTests = Test.testGroup "Outline Configuration Tests"
  [ Test.testCase "createOutlineConfig produces App outline" $ do
      let context = defaultContext
          solverDetails = Map.fromList
...
directorySetupTests = Test.testGroup "Directory Setup Tests"  
...
dependencyFormattingTests = Test.testGroup "Dependency Formatting Tests"
  [ Test.testCase "formatDependencies extracts versions correctly" $ do
      let solverDetails = Map.fromList
            [ (Pkg.core, Solver.Details V.one Map.empty)
...
sourceDirTests = Test.testGroup "Source Directory Tests"
...
integrationTests = Test.testGroup "Integration Tests"
  ...
  , Test.testCase "project creation with custom context" $ do
      let customContext = defaultContext 
