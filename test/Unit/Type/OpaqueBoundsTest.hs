    ...
    [ testCase "extractBoundsFromAliases finds comparable-bounded alias" $ do
      ...
      testCase "extractAllInterfaceBounds collects from multiple interfaces" $ do
        let userIdAlias = Interface.OpaqueAlias (mkBoundedAlias Can.ComparableBound)
            ...
            iface1 = Interface.Interface testPkg Map.empty Map.empty (Map.singleton (Name.fromChars "UserId") userIdAlias) Map.empty Map.empty Map.empty []
            iface2 = Interface.Interface testPkg Map.empty Map.empty (Map.fromList [(Name.fromChars "Score", scoreAlias), (Name.fromChars "Widget", widgetAlias)]) Map.empty Map.empty Map.empty []
