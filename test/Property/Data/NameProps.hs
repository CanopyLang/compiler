...
genQualifiedName = do
  ...
  return (concatMap (\x_ -> ((++)) x_ ".") modules)

testFromCharsToCharsRoundtrip :: TestTree
testFromCharsToCharsRoundtrip =
  testProperty "fromChars/toChars roundtrip" . forAll genValidIdentifier $
    ( \str ->
...
testHasDotConsistency =
  testGroup
    ...
    [ testProperty "hasDot true iff contains dot" . forAll (arbitrary :: Gen String) $
        ( \str ->
            let name = Name.fromChars str
      ...
      testProperty "qualified names have dots" . forAll genQualifiedName $
        ( \str ->
            let name = Name.fromChars str
             in Name.hasDot name
        )
...
testSplitDotsProperties =
  testGroup
    ...
    [ testProperty "splitDots never returns empty list" . forAll (arbitrary :: Gen String) $
        ( \str ->
            let name = Name.fromChars str
                parts = Name.splitDots name
             in not (null parts)
        ),
      testProperty "single names split to singleton" . forAll genValidIdentifier $
        ( \str ->
            let name = Name.fromChars str
                parts = Name.splitDots name
