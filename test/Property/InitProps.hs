...
tests :: TestTree
tests = Test.testGroup "Init Property Tests"
...
configLensProperties = Test.testGroup "InitConfig Lens Properties"
  ...
  , QC.testProperty "configForce lens set/set law" $
      \verbose force skip newForce1 newForce2 ->
        let config = InitConfig verbose force skip
            updated1 = config & configForce Lens..~ newForce1
  ...
  , QC.testProperty "config lens composition is associative" $
      \verbose force skip ->
        let config = InitConfig verbose force skip
            path1 = config & configVerbose Lens..~ True
                          & configForce Lens..~ False
            path2 = (config & configVerbose Lens..~ True)
                          & configForce Lens..~ False
        in path1 === path2
...
contextLensProperties = Test.testGroup "ProjectContext Lens Properties"
  ...
  , QC.testProperty "context lens updates are independent" $
      QC.forAll dependencyMapGen $ \deps ->
        let context = ProjectContext Nothing ["src"] deps Map.empty
            updated = context & contextProjectName Lens..~ Just "Test"
...
errorProperties = Test.testGroup "Error Properties"
...
defaultValueProperties = Test.testGroup "Default Value Properties"
...
validationProperties = Test.testGroup "Validation Properties"
...
dependencyMapProperties = Test.testGroup "Dependency Map Properties"
