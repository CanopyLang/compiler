...
testRealCompilationScenarios = testGroup "real compilation scenarios"
...
testCompilationPipeline = testGroup "compilation pipeline"
...
testErrorIntegration = testGroup "error integration"
        ...
        [result1, result2, result3] -> do
          case (result1, result2, result3) of
            (Left err1, Left err2, Left err3) ->
              assertBool "errors should be deterministic"
                (show err1 == show err2 && show err2 == show err3)
            (Right art1, Right art2, Right art3) ->
              assertBool "successful compilations should be deterministic"
...
testSystemIntegration = testGroup "system integration"
...
createSimpleValidModule = Src.Module
  ...
  , Src._imports = []
  , Src._values = [A.at (A.Position 0 0) (A.Position 0 0) $ Src.Value 
...
createComplexModule = Src.Module
  ...
  , Src._docs = Src.NoDocs A.zero
  , Src._imports = []
  , Src._values = replicate 5 (A.at (A.Position 0 0) (A.Position 0 0) $ Src.Value 
