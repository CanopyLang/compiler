...
tests :: TestTree
tests = testGroup "Terminal Integration Tests"
...
testCompleteCommandWorkflow = testGroup "Complete Command Workflow"
  [ testCase "command creation with all components" $ do
      let stringParser = Terminal.stringParser "name" "project name"
          ...
          flags = Terminal.flags (\a b -> (a, b))
                    Terminal.|-- Terminal.flag "output" stringParser "output directory"
                    Terminal.|-- Terminal.onOff "verbose" "enable verbose output"
          handler _ _ = pure ()
          cmd = Terminal.command "build" (Terminal.Common "Build project") 
                  "Compile and build the project" (Doc.text "build <name> <version>") 
                  args flags handler
      case cmd of
  ...
  , testCase "command with no arguments and no flags" $ do
      let args = Terminal.noArgs
          ...
          handler _ _ = pure ()
          cmd = Terminal.command "help" Terminal.Uncommon 
                  "Show help information" Doc.empty args flags handler
      case cmd of
  ...
  , testCase "command with complex argument patterns" $ do
      let fileParser = Terminal.fileParser [".hs", ".canopy"]
          ...
          flags = Terminal.noFlags
          handler _ _ = pure ()
          cmd = Terminal.command "process" (Terminal.Common "Process files") 
                  "Process various file types" Doc.empty complexArgs flags handler
      case cmd of
...
testArgumentFlagIntegration = testGroup "Argument Flag Integration"
...
testParserChainIntegration = testGroup "Parser Chain Integration"
...
testComplexScenarios = testGroup "Complex Scenarios"
  [ testCase "build command simulation" $ do
      let srcParser = Terminal.fileParser [".hs", ".canopy"]
          ...
          flags = Terminal.flags (\a b c -> (a, b, c))
                    Terminal.|-- outputFlag
  ...
  , testCase "install command simulation" $ do
      let packageParser = Terminal.stringParser "package" "package name"
          ...
          flags = Terminal.flags (,) 
                    ...
                    Terminal.|-- saveFlag
          handler _ _ = pure ()
          installCmd = Terminal.command "install" Terminal.Uncommon
                         "Install packages" Doc.empty args flags handler
      case installCmd of
