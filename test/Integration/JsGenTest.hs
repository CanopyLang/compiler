...
testDevGenSingleFile =
  testCase "Generate.dev produces JS with multiple functions" . withSystemTempDirectory "can-js-dev" $
    ( \tmp -> do
...
testProdGenSingleFile =
  testCase "Generate.prod produces JS with multiple functions" . withSystemTempDirectory "can-js-prod" $
    ( \tmp -> do
        setupPkgProject tmp sampleModule
        details <- BW.withScope $ \scope -> do
          e <- Details.load Reporting.silent scope tmp
...
toChars = fmap (toEnum . fromEnum) . BL.unpack
