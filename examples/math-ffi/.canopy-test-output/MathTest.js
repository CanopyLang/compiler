...
var $user$project$MathTest$allTests = A2( $user$project$Test$describe,'Math FFI Complete Test Suite', _List_fromArray([ $user$project$MathTest$factorialTests , $user$project$MathTest$fibonacciTests , $user$project$MathTest$gcdTests , $user$project$MathTest$lcmTests , $user$project$MathTest$primeTests , $user$project$MathTest$powerTests , $user$project$MathTest$sqrtTests , $user$project$MathTest$angleConversionTests , $user$project$MathTest$distanceTests]));
...
var $user$project$Test$formatFailure = function(result){ return ('FAIL ' +( result.description +(': ' + result.message)));};
...
var $user$project$Test$reasonToString = function(reason){ switch( reason.$){ case 'Equality' :var label = reason.a;var expected = reason.b;var actual = reason.c; return ( label +(': Expected ' +( expected +(', got ' + actual)))); case 'Comparison' :var label = reason.a;var actual = reason.b;var op = reason.c;var threshold = reason.d; return ( label +(': ' +( actual +(' ' +( op +(' ' + threshold)))))); default :var message = reason.a; return ( message);}};
var $user$project$Test$expectationToMessage = function(expectation){if ( expectation.$ ==='Pass' ) return ''; else{var reason = expectation.a; return ( $user$project$Test$reasonToString( reason));}};
var $elm$core$List$isEmpty = function(xs){if (! xs.b ) return true; else return false;};
var $user$project$Test$run = function(testCase){if ( testCase.$ ==='UnitTest' ){var description = testCase.a;var expectationFn = testCase.b;var result = expectationFn( _Utils_Tuple0);if ( result.$ ==='Pass' ) return ({status :'PASS',message :'',duration :0,description : description}); else return ({status :'FAIL',message : $user$project$Test$expectationToMessage( result),duration :0,description : description});} else{var description = testCase.a;var tests = testCase.b;var results = A2( $elm$core$List$map, $user$project$Test$run, tests);var failures = A2( $elm$core$List$filter, function(r){ return ( r.status ==='FAIL');}, results); return ({status : $elm$core$List$isEmpty( failures)?'PASS':'FAIL',message : $elm$core$String$fromInt( $elm$core$List$length( failures)) +(' of ' +( $elm$core$String$fromInt( $elm$core$List$length( results)) +' tests failed')),duration :0,description : description});}};
var $user$project$Test$runTests = function(tests){var results = A2( $elm$core$List$map, $user$project$Test$run, tests);var totalTests = $elm$core$List$length( results);var passedTests = $elm$core$List$length( A2( $elm$core$List$filter, function(r){ return ( r.status ==='PASS');}, results));var failedTests = totalTests - passedTests; return ('Test Results:\n' +('Total: ' +( $elm$core$String$fromInt( totalTests) +('\n' +('Passed: ' +( $elm$core$String$fromInt( passedTests) +('\n' +('Failed: ' +( $elm$core$String$fromInt( failedTests) +('\n' +( failedTests >0)?('\nFailures:\n' + A2( $elm$core$String$join,'\n', A2( $elm$core$List$map, $user$project$Test$formatFailure, A2( $elm$core$List$filter, function(r){ return ( r.status ==='FAIL');}, results)))):'\nAll tests passed!'))))))))));};
var $elm$virtual_dom$VirtualDom$text = _VirtualDom_text;
var $elm$html$Html$text = $elm$virtual_dom$VirtualDom$text;
var $user$project$MathTest$main = function(){var results = $user$project$Test$runTests( _List_fromArray([ $user$project$MathTest$allTests])); return ( A2( $elm$html$Html$pre, _List_Nil, _List_fromArray([ $elm$html$Html$text( results)])));}();
...
_Platform_export({'MathTest':{'init': _VirtualDom_init( $user$project$MathTest$main)(0)(0)}});scope['Canopy'] = scope['Elm'];
