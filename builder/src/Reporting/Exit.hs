...
diffToReport diff =
  case diff of
    ...
    DiffCustomReposDataProblem _ ->
      ...
      Help.report
        "DIFF WITH BAD CUSTOM REPOSITORY CONFIGURATION"
        Nothing
        "blah blah blah"
        []
...
bumpToReport bump =
  case bump of
    ...
    BumpCustomRepositoryDataProblem _ ->
      ...
      Help.report
        "BUMP WITH BAD CUSTOM REPOSITORY CONFIGURATION"
        Nothing
        "blah blah blah"
        []
...
publishToReport publish =
  case publish of
      ...
      let vsn = V.toChars version
       in Help.docReport
            ...
            [ D.vcat
              ...
              ((D.indent 4 . D.dullyellow) . D.vcat) . fmap D.fromChars $
        ...
        [ D.indent 4 . D.dullyellow $ D.fromChars url,
          D.reflow
            ( "I got the data back, but it was not what I was expecting. The response\
              \ body contains "
                <> (show (BS.length body) <> (" bytes. Here is the " <> (if BS.length body <= 76 then "whole thing:" else "beginning:")))
            ),
    ...
    PublishWithNoRepositoryLocalName ->
      Help.report
        "PUBLISH WITH NO REPOSITORY URL"
        Nothing
    ...
    PublishCustomRepositoryConfigDataError _ ->
      -- FIXME: Add actual error message here
      Help.report
        "PUBLISH WITH BAD CUSTOM REPOSITORY CONFIGURATION"
        Nothing
        "blah blah blah"
        []
...
toHttpErrorReport title err context =
  let toHttpReport intro url details =
...
toRedirectDoc response =
  let (HTTP.Status code message) = HTTP.responseStatus response
   in case List.lookup HTTP.hLocation (HTTP.responseHeaders response) of
