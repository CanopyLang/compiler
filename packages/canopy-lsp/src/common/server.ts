...
export class Server implements ILanguageServer {
  ...
  constructor(
    ...
    if (uri) {
      ...
      if (!elmJsons) {
        this.connection.window.showErrorMessage(
          "Unable to find canopy.json or elm.json files.",
        );
        return;
      }
      ...
      if (elmJsons.length > 0) {
        this.connection.console.info(
          `Found ${
            elmJsons.length
          } canopy.json/elm.json files for workspace ${uri.toString()}`,
        );
        ...
        this.connection.window.showErrorMessage(
          "No canopy.json or elm.json found. Please run 'canopy init' or 'elm init' in your main directory.",
        );
        this.connection.console.info(`No canopy.json or elm.json found`);
      }
    } else {
