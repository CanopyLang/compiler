...
export class Program implements IProgram {
  ...
  private async initWorkspace(): Promise<void> {
    ...
    const pathToCanopyJson = Utils.joinPath(this.rootPath, "canopy.json");
    const pathToElmJson = Utils.joinPath(this.rootPath, "elm.json");

    // Check for canopy.json first, then elm.json
    const configExists = await Promise.all([
      this.host.readFile(pathToCanopyJson).then(() => true).catch(() => false),
      this.host.readFile(pathToElmJson).then(() => true).catch(() => false)
    ]);

    const pathToConfigFile = configExists[0] ? pathToCanopyJson : pathToElmJson;
    const configFileName = configExists[0] ? "canopy.json" : "elm.json";

    this.connection.console.info(
      `Reading ${configFileName} from ${pathToConfigFile.toString()}`,
    );

    if (!this.filesWatching.has(pathToConfigFile.toString())) {
      this.disposables.push(
        this.host.watchFile(pathToConfigFile, () => {
          void this.connection.window
            .createWorkDoneProgress()
            .then((progress) => {
              progress.begin("Restarting Canopy Language Server", 0);
      ...
      this.filesWatching.add(pathToConfigFile.toString());
    }
    ...
    try {
      ...
      try {
        ...
        if (error instanceof Error && error.stack) {
          this.connection.window.showErrorMessage(
            `Failed constructing ElmPackageCache for ${pathToConfigFile.toString()}:\n${
              error.stack
            }`,
          );
        }
        ...
        this.connection.window.showInformationMessage(
          "The package cache is probably broken. Try a restart after removing '~/.elm' or '%APPDATA%\\elm'." +
            "If the error still occurs, try running 'canopy init' in a different folder." +
            "If the error appears again, check your PATH for multiple canopy installations and verify your installed version",
        );

        throw error;
      }

      this.rootProject = await this.loadRootProject(pathToConfigFile);
      ...
      if (elmFilePaths.every((a) => a.project !== this.rootProject)) {
        this.connection.window.showErrorMessage(
          "The path or paths you entered in the 'source-directories' field of your configuration file does not contain any canopy or elm files.",
        );
      }
  ...
  private async loadRootProject(elmJsonPath: URI): Promise<ElmProject> {
    ...
    if (elmJson.type === "application") {
      ...
      if (!solvedVersions) {
        this.connection.window.showErrorMessage(
          "There is a problem with the configuration file. Could not solve dependencies with the given constraints. Try running `canopy make` to install missing dependencies.",
        );
