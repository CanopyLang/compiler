...
addNewPkgDependency env pkg outline = do
  ...
  case Registry.getVersions' registry pkg of
    Nothing -> throwPkgUnknownPackageError connection pkg []
    Just _ -> solvePkgDependency solverCtx pkg outline
...
data PkgSolverContext = PkgSolverContext
  { _pscCache :: !Stuff.PackageCache,
    ...
    _pscRegistry :: !Registry.CanopyRegistries
  }
...
throwPkgUnknownPackageError _connection pkg suggestions =
