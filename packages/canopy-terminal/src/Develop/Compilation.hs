...
loadProjectDetails scope root = do
  result <- Task.io (Details.load Reporting.silent scope root)
  either (Task.throw . Exit.ReactorBadDetails) pure result
...
buildSourceArtifacts root details path = do
  result <- Task.io (Build.fromPaths Reporting.silent root details [path])
  either (Task.throw . Exit.ReactorBadBuild) pure result
