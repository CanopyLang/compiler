...
-- packages (canopy\/core, canopy\/html, etc.) are available for compilation.
--
...
reportSummary located missing localCompiled = do
  ...
  if missing > 0
    then do
      ...
      Print.println [c|  1. Run '{green|canopy link}' in each package directory to create symlinks|]
      Print.println [c|  2. Then run '{green|canopy setup}' again to compile from source|]
    else do
      Print.newline
