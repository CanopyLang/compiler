...
import Publish.Types (Env (..), GoodVersion (..), envCache, envManager)
...
badSummary summary =
  let summaryText = Text.pack (Json.toChars summary)
   in null (Json.toChars summary) || Outline.defaultSummary == summaryText
...
verifyBuild root = Task.io (BW.withScope (\scope -> Task.run (buildAndGenerateDocs root scope))) >>= either Task.throw pure
...
loadProjectDetails scope projectRoot = do
  result <- Task.io (Details.load Reporting.silent scope projectRoot)
  either (Task.throw . Exit.PublishBadDetails) pure result
...
validateInitialVersion version =
  pure $
    if version == Version.one
      ...
      else Left (Exit.PublishNotInitialVersion (Version.toChars version))
...
validateVersionBump env package version docs knownVersions@(Registry.KnownVersions latest previous) =
  if version == latest || elem version previous
    then pure (Left (Exit.PublishAlreadyPublished (Version.toChars version)))
...
verifyBump env pkg vsn newDocs knownVersions =
  maybe
    (pure (Left (Exit.PublishInvalidBump (Version.toChars vsn))))
...
validateBumpAgainstDocs env package oldVer newVer mag docs = do
  result <- Diff.getDocs (env ^. envCache) (env ^. envManager) package oldVer
  either
    (\dp -> pure (Left (Exit.PublishCannotGetDocs (show dp))))
  ...
  let changes = Diff.diff oldDocs docs
   ...
   in pure $
        if newVer == realNew
          ...
          else Left (Exit.PublishBadBump (Version.toChars oldVer))
