...
createOutlineConfig context solverDetails =
  let solution = extractVersions solverDetails
      sourceDirs = nonEmptyListToList (createSourceDirList (context ^. contextSourceDirs))
      ...
      depsConstraints = context ^. contextDependencies
...
-- | Convert NonEmptyList to regular list.
nonEmptyListToList :: NE.List a -> [a]
nonEmptyListToList (NE.List x xs) = x : xs
