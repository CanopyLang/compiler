...
analyzeDocs env = do
  ...
  oldDocs <- case oldDocsResult of
    Left docsProblem -> Task.throw (Exit.BumpCannotFindDocs (show docsProblem))
...
--
-- @since 0.19.1
getDocs :: Env -> Name -> Version -> IO (Either String Documentation)
getDocs env pkg vsn =
  Diff.getDocs (env ^. envCache) (env ^. envManager) pkg vsn
...
loadPackageDetails root = do
  result <- Task.io (BackgroundWriter.withScope loadDetails)
  either (Task.throw . Exit.BumpBadDetails) pure result
  where
