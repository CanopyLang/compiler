...
renderScript loaders =
  Text.unlines
    ...
    , "    process.exit(1);"
    , "  }"
    , "  const dom = new JSDOM('<!DOCTYPE html><html><body><div id=\"app\" data-canopy-hydrate></div></body></html>');"
    ...
    , "  const dataScript = dom.window.document.createElement('script');"
    , "  dataScript.id = '__CANOPY_DATA__';"
    , "  dataScript.type = 'application/json';"
    , "  dataScript.textContent = JSON.stringify(loaderData);"
    , "  dom.window.document.body.appendChild(dataScript);"
