...
--
-- If no paths are given, scans @tests\/@ and @test\/@ for @.can@ files.
...
compileTestFiles root testFiles = do
  let pkg = Pkg.dummyName
      srcDirs = [Compiler.RelativeSrcDir "src", Compiler.RelativeSrcDir "tests", Compiler.RelativeSrcDir "test"]
...
-- Also applies post-processing to fix JavaScript syntax issues.
...
artifactsToJavaScript artifacts =
  ...
  in postProcessJavaScript rawJs

-- | Post-process JavaScript to fix syntax issues.
--
-- The code generator produces 'elseif' which is valid in some contexts
-- but Node.js requires 'else if' with a space.
...
--
-- Initializes Canopy test modules and extracts their rendered text output.
-- Test modules render results as HTML text which we capture and display.
generateTestHarness :: Maybe String -> String
generateTestHarness _maybeFilter =
  unlines
    [ "// Canopy test harness - auto-generated by canopy test",
      "",
      "// Simple virtual DOM text extractor",
      "function extractText(vdom) {",
      "  if (typeof vdom === 'string') return vdom;",
      "  if (vdom && vdom.text) return vdom.text;",
      "  if (vdom && vdom.kids) {",
      "    return vdom.kids.map(extractText).join('');",
      "  }",
      "  if (vdom && vdom.c) {", -- children array
      "    return vdom.c.map(extractText).join('');",
      "  }",
      ...
      "}",
      "",
      "// Find and run all Canopy test modules",
      "// Note: FFI files may overwrite module.exports, so we look in global/window instead",
      "var globalScope = (typeof global !== 'undefined' ? global : (typeof window !== 'undefined' ? window : this));",
      "var hasTests = false;",
      "var anyFailed = false;",
      "",
      "// Look for test modules in Elm/Canopy namespace",
      "var testScope = globalScope.Canopy || globalScope.Elm || {};",
      "",
      ...
      "    hasTests = true;",
      "    try {",
      "      // Initialize with a mock DOM node",
      "      var mockNode = { appendChild: function(){}, removeChild: function(){} };",
      "      var app = mod.init({ node: mockNode });",
      "      ",
      "      // Extract the virtual DOM text if available",
      "      if (app && app._impl && app._impl.view) {",
      "        var vdom = app._impl.view(app._impl.model);",
      "        var text = extractText(vdom);",
      "        console.log(text);",
      "      }",
      "    } catch (e) {",
      "      // Try alternative initialization for static views",
      "      try {",
      "        var staticMain = testScope[moduleName + '$main'];",
      "        if (staticMain) {",
      "          var text = extractText(staticMain);",
      "          console.log(text);",
      "        }",
      "      } catch (e2) {",
      "        console.log('Test module ' + moduleName + ' failed to initialize: ' + e.message);",
      "        anyFailed = true;",
      "      }",
      "    }",
      "  }",
      "});",
      "",
      "// Check for test failure indicators in output",
      "if (!hasTests) {",
