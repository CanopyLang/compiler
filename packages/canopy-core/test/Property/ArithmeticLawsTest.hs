...
-- * ✅ Invariant testing across compiler phases
-- * ❌ NO meaningless properties (x == x)
-- * ❌ NO trivial properties that always pass
--
-- @since 0.19.1
...
arithOpProperties = testGroup "ArithOp Properties"
  [ testProperty "ArithOp Eq is reflexive" $ \op ->
      (op :: Can.ArithOp) == op
  ...
  , testProperty "ArithOp show is non-empty" $ \op ->
      not (null (show (op :: Can.ArithOp)))

  , testProperty "ArithOp show contains type name" $ \op ->
      let shown = show (op :: Can.ArithOp)
      in any (`elem` ["Add", "Sub", "Mul", "Div"]) (words shown)
  ]
...
binopKindProperties = testGroup "BinopKind Properties"
  [ testProperty "BinopKind Eq is reflexive" $ \kind ->
      (kind :: Can.BinopKind) == kind

  , testProperty "BinopKind Eq is symmetric" $ \kind1 kind2 ->
      (kind1 == kind2) == ((kind2 :: Can.BinopKind) == (kind1 :: Can.BinopKind))

  , testProperty "BinopKind show is non-empty" $ \kind ->
      not (null (show (kind :: Can.BinopKind)))

  , testProperty "NativeArith classification preserved in show" $ \op ->
      let kind = Can.NativeArith op
          shown = show kind
      in "NativeArith" `elem` words shown

  , testProperty "UserDefined classification preserved in show" $
...
binaryRoundtripProperties = testGroup "Binary Roundtrip Properties"
  ...
  , testProperty "ArithOp encoding is compact" $ \op ->
      let encoded = Binary.encode (op :: Can.ArithOp)
      in Binary.length encoded <= 10  -- Should be very small
  ]
...
mathematicalLawProperties = testGroup "Mathematical Law Properties"
  [ testProperty "Addition operator representation is consistent" $
      Can.Add == Can.Add

  , testProperty "Subtraction operator representation is consistent" $
      Can.Sub == Can.Sub

  , testProperty "Multiplication operator representation is consistent" $
      Can.Mul == Can.Mul

  , testProperty "Division operator representation is consistent" $
      Can.Div == Can.Div
  ...
  , testProperty "Same operators have EQ ordering" $ \op ->
