...
fromInterface name (Interface.Interface pkg _ unions aliases _ _ _ _) =
...
fromDependencyInterface home di =
  Types . Map.singleton home $
    ( case di of
        Interface.Public (Interface.Interface _ _ unions aliases _ _ _ _) ->
...
extractAlias (Types dict) (Opt.Global home name) =
  let types_ =
      ...
      (Can.Alias args _ aliasType _ _) =
        maybe
          (InternalError.report "Canopy.Compiler.Type.Extract.extractAlias"
