        ...
        badPatterns ->
          let firstPatterns =
                map
                  ...
                  badPatterns
           in Incomplete region context firstPatterns : errors
          ...
          let ctors = collectCtors matrix
           ...
           in if numSeen == 0
                then
                  ...
                  let alts@(Can.Union _ _ altList numAlts _ _) = snd (Map.findMin ctors)
  ...
  let ctors = collectCtors matrix
   ...
   in if numSeen == 0
        then No
          ...
          let (Can.Union _ _ alts numAlts _ _) = snd (Map.findMin ctors)
