...
flatten pathPattern@(path, Ann.At region pattern) otherPathPatterns =
  case pattern of
    ...
    Can.PAnything ->
      pathPattern : otherPathPatterns
    Can.PCtor _ _ (Can.Union _ _ _ numAlts _ _) _ _ ctorArgs ->
      if numAlts == 1
...
testAtPath selectedPath (Branch _ pathPatterns) =
  case List.lookup selectedPath pathPatterns of
    ...
    Just (Ann.At _ pattern) ->
      case pattern of
        Can.PCtor home _ (Can.Union _ _ _ numAlts opts _) name index _ ->
...
toRelevantBranch test path branch@(Branch goal pathPatterns) =
  case extract path pathPatterns of
    Found start (Ann.At region pattern) end ->
      case pattern of
        Can.PCtor _ _ (Can.Union _ _ _ numAlts _ _) name _ ctorArgs ->
          case test of
