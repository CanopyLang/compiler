...
-- Links, ensuring each branch starts with truly fresh polymorphic types.

solveCaseBranchesIsolated :: SolveConfig -> [Constraint] -> IO State
solveCaseBranchesIsolated config branchConstraints = do
  envSnapshot <- snapshotPolymorphicVariables (config ^. solveEnv)
  foldM (solveOneBranch config envSnapshot) (config ^. solveState) branchConstraints

solveOneBranch :: SolveConfig -> Map Name.Name Variable -> State -> Constraint -> IO State
solveOneBranch config envSnapshot currentState branchConstraint = do
  restoredEnv <- restoreEnvironmentFromSnapshot envSnapshot (config ^. solveEnv)
  let branchConfig = config & solveEnv .~ restoredEnv & solveState .~ currentState
  solve branchConfig branchConstraint

snapshotPolymorphicVariables :: Env -> IO (Map Name.Name Variable)
snapshotPolymorphicVariables env = do
  let envList = Map.toList env
  polymorphicFunctionPairs <- filterPolymorphicFunctions envList
  return (Map.fromList polymorphicFunctionPairs)
  where
    filterPolymorphicFunctions [] = return []
    filterPolymorphicFunctions ((name, var) : rest) = do
      isPolyFunc <- isPolymorphicFunction var
      remaining <- filterPolymorphicFunctions rest
      if isPolyFunc
        then return ((name, var) : remaining)
        else return remaining

isPolymorphicFunction :: Variable -> IO Bool
isPolymorphicFunction var = do
  (Descriptor content rank _ _) <- UF.get var
  case (rank == noRank, content) of
    (True, Structure (Fun1 _ _)) -> return True  -- Polymorphic function type
    (True, Alias _ _ _ _) -> return True  -- Polymorphic alias (might wrap function)
    _ -> return False  -- Not a polymorphic function, don't isolate

restoreEnvironmentFromSnapshot :: Map Name.Name Variable -> Env -> IO Env
restoreEnvironmentFromSnapshot snapshot baseEnv =
  Map.traverseWithKey (restoreVariable snapshot) baseEnv

restoreVariable :: Map Name.Name Variable -> Name.Name -> Variable -> IO Variable
restoreVariable snapshot name var =
  case Map.lookup name snapshot of
    Nothing -> return var
    Just originalVar -> deepCloneVariable originalVar

deepCloneVariable :: Variable -> IO Variable
deepCloneVariable var = do
  (Descriptor content rank mark _) <- UF.get var
  case content of
    Structure flatType -> do
      -- Deep clone the entire structure recursively
      -- This prevents ANY Link pollution from previous branches
      clonedFlatType <- traverseFlatType deepCloneVariable flatType
      UF.fresh (Descriptor (Structure clonedFlatType) rank mark Nothing)
    Alias home name args realVar -> do
      -- Deep clone alias arguments and real variable
      clonedArgs <- traverse (traverse deepCloneVariable) args
      clonedRealVar <- deepCloneVariable realVar
      UF.fresh (Descriptor (Alias home name clonedArgs clonedRealVar) rank mark Nothing)
    FlexVar maybeName ->
      -- Create fresh flex variable for complete independence
      UF.fresh (Descriptor (FlexVar maybeName) rank mark Nothing)
    FlexSuper super maybeName ->
      UF.fresh (Descriptor (FlexSuper super maybeName) rank mark Nothing)
    RigidVar name ->
      UF.fresh (Descriptor (RigidVar name) rank mark Nothing)
    RigidSuper super name ->
      UF.fresh (Descriptor (RigidSuper super name) rank mark Nothing)
    Error ->
      UF.fresh (Descriptor Error rank mark Nothing)
