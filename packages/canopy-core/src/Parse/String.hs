...
character :: (Row -> Col -> x) -> (SyntaxError.Char -> Row -> Col -> x) -> Parser x ES.String
character toExpectation toError =
  Parse.Parser $ \(Parse.State src pos end indent row col) cok _ cerr eerr ->
    if pos >= end || Parse.unsafeIndex pos /= 0x27 {- ' -}
      then eerr row col toExpectation
        ...
        Good newPos newCol numChars mostRecent ->
          if numChars /= 1
            then cerr row col (toError (SyntaxError.CharNotString (fromIntegral (newCol - col))))
            else
              let !newState = Parse.State src newPos end indent row newCol
                  !char = ES.fromChunks [mostRecent]
               in cok char newState
        CharEndless newCol ->
          cerr row newCol (toError SyntaxError.CharEndless)
                    ...
                    then chompChar (plusPtr pos 1) end row (col + 1) (numChars + 1) doubleQuote
                      ...
                      if word == 0x5C {- \ -}
                        then case eatEscape (plusPtr pos 1) end row col of
                          ...
                          let !width = Parse.getCharWidth word
                              !newPos = plusPtr pos width
                           in chompChar newPos end row (col + 1) (numChars + 1) (ES.Slice pos width)

-- STRINGS

string :: (Row -> Col -> x) -> (SyntaxError.String -> Row -> Col -> x) -> Parser x ES.String
...
isDoubleQuote pos end =
  pos < end && Parse.unsafeIndex pos == 0x22 {- " -}

data StringResult
  = Ok (Ptr Word8) Row Col !ES.String
  | Err Row Col SyntaxError.String
...
singleString pos end row col initialPos revChunks =
  if pos >= end
    then Err row col SyntaxError.StringEndless_Single
    else
      let !word = Parse.unsafeIndex pos
       in if word == 0x22 {- " -}
            then
                    ...
                    then
                      ...
                      if word == 0x5C {- \ -}
                        then case eatEscape (plusPtr pos 1) end row col of
                          ...
                          EscapeProblem r c x ->
                            Err r c (SyntaxError.StringEscape x)
                          EscapeEndOfFile ->
                            Err row (col + 1) SyntaxError.StringEndless_Single
                        else
...
multiString pos end row col initialPos sr sc revChunks =
  if pos >= end
    then Err sr sc SyntaxError.StringEndless_Multi
    else
      let !word = Parse.unsafeIndex pos
                          ...
                          if word == 0x5C {- \ -}
                            then case eatEscape (plusPtr pos 1) end row col of
                              ...
                              EscapeProblem r c x ->
                                Err r c (SyntaxError.StringEscape x)
                              EscapeEndOfFile ->
                                Err sr sc SyntaxError.StringEndless_Multi
                            else
                              let !newPos = plusPtr pos (Parse.getCharWidth word)
...
data Escape
  ...
  | EscapeEndOfFile
  | EscapeProblem Row Col SyntaxError.Escape
...
eatEscape pos end row col =
  if pos >= end
    then EscapeEndOfFile
      ...
      _ -> EscapeProblem row col SyntaxError.EscapeUnknown

eatUnicode :: Ptr Word8 -> Ptr Word8 -> Row -> Col -> Escape
eatUnicode pos end row col =
  if pos >= end || Parse.unsafeIndex pos /= 0x7B {- { -}
    then EscapeProblem row col (SyntaxError.BadUnicodeFormat 2)
    else
      let !digitPos = plusPtr pos 1
          (# newPos, code #) = Number.chompHex digitPos end
          !numDigits = minusPtr newPos digitPos
       in if newPos >= end || Parse.unsafeIndex newPos /= 0x7D {- } -}
            then EscapeProblem row col $ SyntaxError.BadUnicodeFormat (2 + fromIntegral (minusPtr newPos pos))
            else
              if code < 0 || 0x10FFFF < code
                then EscapeProblem row col $ SyntaxError.BadUnicodeCode (3 + fromIntegral (minusPtr newPos pos))
                else
                  if numDigits < 4 || 6 < numDigits
                    then
                      EscapeProblem row col $
                        SyntaxError.BadUnicodeLength
