...
generateFFIValidators mode ffiInfos usedFuncs =
  if Map.null ffiInfos
     then mempty
            ...
            <> Validator.generateAllValidatorsDeduped config allReturnTypes
  where
...
formatFFIWithBindings mode graph usedFuncs fileResults resolvedNeeded _key info acc
  ...
  where
    ...
    --
    -- Also use the full-file path when filteredFunctions is empty: this happens
    -- for kernel-bridging FFI files (alias ends with "FFI", e.g. JsonFFI) whose
    -- callers emit Opt.VarRuntime references that bypass the global-graph tracker,
    -- leaving neededFuncNames empty and filteredFunctions empty. In that case
    -- neededFunctions already falls back to ALL functions for the return object,
    -- so the body must also use fullAST to keep body and return consistent.
    -- Without this guard, cross-file dependency propagation can add a non-empty
    -- allNeeded for json.js (internal helpers referenced by other FFI files),
    -- making treeShaken = False, which emits only those helper blocks in the body
    -- while the return still includes all public API names — producing
    -- "succeed is not defined" errors at runtime.
    treeShaken = Map.null ffiRegistry || Set.null allNeeded || null filteredFunctions
...
makeBindingStmts varName aliasJsName propJsName valueExpr =
  ...
  , JS.ExprStmtWithSemi (JS.Assign (JS.LDot (JS.Ref aliasJsName) propJsName) (JS.Ref varName))
  ]
