...
generate mode expression =
  case expression of
    ...
    Opt.VarGlobal (Opt.Global home name) ->
      -- Check if this is an FFI module (identified by author/project package)
      let pkg = ModuleName._package home
          moduleName = ModuleName._module home
      in if Pkg._author pkg == Pkg._author Pkg.dummyName && Pkg._project pkg == Pkg._project Pkg.dummyName
    ...
    Opt.VarBox (Opt.Global home name) ->
      JsExpr . JS.Ref $
        ( case mode of
            Mode.Dev _ _ _ -> JsName.fromGlobal home name
...
generateCtor mode (Opt.Global home name) index arity =
  let argNames =
      ...
      ctorTag =
        case mode of
          Mode.Dev _ _ _ -> JS.String (Name.toBuilder name)
  ...
  let value = pathToJsExpr mode root path
   in case test of
        DT.IsCtor home name index _ opts ->
          let tag =
                case mode of
                  Mode.Dev _ _ _ -> JS.Access value JsName.dollar
                  Mode.Prod {} ->
                    case opts of
        ...
        DT.IsChr char ->
          strictEq (JS.String (Utf8.toBuilder char)) $
            case mode of
              Mode.Dev _ _ _ -> JS.Call (JS.Access value (JsName.fromLocal "valueOf")) []
              Mode.Prod {} -> value
        DT.IsStr string ->
