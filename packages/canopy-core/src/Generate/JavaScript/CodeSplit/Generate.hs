...
handleProdChunk mode builder =
  case mode of
    Mode.Dev _ _ _ -> "_UNUSED" <> builder
    Mode.Prod {} -> builder
...
emitEnum mode global@(Opt.Global home name) index =
  JS.Var (JsName.fromGlobal home name) $
    case mode of
      Mode.Dev _ _ _ -> Expr.codeToExpr (Expr.generateCtor mode global index 0)
...
emitBox mode global@(Opt.Global home name) =
  JS.Var (JsName.fromGlobal home name) $
    case mode of
      Mode.Dev _ _ _ -> Expr.codeToExpr (Expr.generateCtor mode global Index.first 1)
