...
generateFFIContent mode graph ffiInfos =
  if Map.null ffiInfos
     then mempty
     else mconcat (map B.stringUtf8 parts) <> validators
  where
    parts =
    ...
    validators =
      if Mode.isFFIStrict mode
        then generateFFIValidators ffiInfos
        else mempty
...
generateFFIValidators ffiInfos =
  ...
  where
    ...
    collectValidators :: String -> FFIInfo -> [String] -> [String]
    collectValidators _key info acc =
      let content = Text.unpack (_ffiContent info)
          ...
          validators = concatMap generateValidatorForFunction functions
      in validators ++ acc

    generateValidatorForFunction :: (String, String) -> [String]
    generateValidatorForFunction (_funcName, typeStr) =
      case Validator.parseReturnType (Text.pack typeStr) of
        Just returnType ->
          [Text.unpack (Validator.generateAllValidators Validator.defaultConfig returnType)]
        Nothing -> []
...
generateFFIBindingsFromInfo :: Mode.Mode -> Graph -> String -> FFIInfo -> [String] -> [String]
...
extractFFIFunctionBindings :: Mode.Mode -> Graph -> String -> String -> String -> [String]
...
generateFunctionBinding :: Mode.Mode -> Graph -> String -> String -> (String, String) -> [String]
...
generateSimpleBinding :: String -> String -> String -> Int -> [String]
generateSimpleBinding jsVarName alias funcName arity =
  let wrapper = if arity <= 1 then "" else "F" ++ show arity ++ "("
...
generateValidatedBinding :: String -> String -> String -> Int -> String -> String -> [String]
generateValidatedBinding jsVarName alias funcName arity canopyType callPath =
  let args = if arity <= 0 then [] else map (\i -> "_" ++ show i) [0 .. arity - 1]
      argList = intercalate ", " args
      returnType = extractReturnType canopyType
      validatorExpr = typeToValidator returnType
      wrappedCall = funcName ++ "(" ++ argList ++ ")"
      validatedCall = validatorExpr ++ "(" ++ wrappedCall ++ ", " ++ callPath ++ ")"
      funcBody = "function(" ++ argList ++ ") { return " ++ validatedCall ++ "; }"
      wrapper = if arity <= 1 then "" else "F" ++ show arity ++ "("
      closing = if arity <= 1 then "" else ")"
      binding = "var " ++ jsVarName ++ " = " ++ wrapper ++ funcBody ++ closing ++ ";"
      namespaceBinding = alias ++ "." ++ funcName ++ " = " ++ jsVarName ++ ";"
...
-- | Convert a type string to a $validate expression
typeToValidator :: String -> String
typeToValidator typeStr =
  case Validator.parseFFIType (Text.pack typeStr) of
    Just ffiType -> ffiTypeToValidator ffiType
    Nothing -> "$validate.Any"  -- Fallback for unparseable types

-- | Convert FFIType to $validate expression
ffiTypeToValidator :: Validator.FFIType -> String
