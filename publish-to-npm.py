...
def rewrite_versions_of_optional_dependencies(package_json, version):
    package_json_copy = copy.deepcopy(package_json)
    package_json_copy["optionalDependencies"]["@zokka/zokka-binary-darwin_x64"] = (
        version
    )
    package_json_copy["optionalDependencies"]["@zokka/zokka-binary-darwin_arm64"] = (
        version
    )
...
try:
    ...
    raise Exception(
        "The NPM_TOKEN environment variable must be set otherwise we cannot publish to npm!"
    )
...
def copy_and_chmod_file(source, destination):
    try:
        ...
        print(
            f"You are asking to copy {source} to {destination}, which are just the exact same file path, so no copying is taking place and just leave the file as-is."
        )
        pass
...
copy_and_chmod_file(
    darwin_arm64_binary_source_location, darwin_arm64_directory + "/zokka"
)
...
for directory in [
    darwin_x86_directory,
    darwin_arm64_directory,
    windows_directory,
    linux_directory,
]:
    ...
    if "alpha" in new_version:
        subprocess.run(
            ["npm", "publish", "--tag", "alpha"] + additional_npm_args, cwd=directory
        )
    elif "beta" in new_version:
        subprocess.run(
            ["npm", "publish", "--tag", "beta"] + additional_npm_args, cwd=directory
        )
    else:
...
with open(top_level_npm_directory + "package.json", "r+") as f:
    ...
    new_package_metadata = rewrite_version_of_package_json(
        rewrite_versions_of_optional_dependencies(package_metadata, new_version),
        new_version,
    )
    f.seek(0)
...
if "alpha" in new_version:
    subprocess.run(
        ["npm", "publish", "--tag", "alpha"] + additional_npm_args,
        cwd=top_level_npm_directory,
    )
    ...
    if dry_run:
        print(
            "Skipping setting published package as latest version because this is a dry-run."
        )
    else:
        subprocess.run(
            ["npm", "dist-tag", "add", f"zokka@{new_version}", "latest"]
            + additional_npm_args,
            cwd=top_level_npm_directory,
        )
elif "beta" in new_version:
    subprocess.run(
        ["npm", "publish", "--tag", "beta"] + additional_npm_args,
        cwd=top_level_npm_directory,
    )
    # Note that according to
    # https://docs.npmjs.com/cli/v9/commands/npm-publish#dry-run dist-tag does
    ...
    subprocess.run(
        ["npm", "publish"] + additional_npm_args, cwd=top_level_npm_directory
    )
