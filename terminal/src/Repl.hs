  ...
  do
    ...
    case maybeLine of
      Nothing ->
        return Exit
      Just chars ->
        let lines = Lines (stripLegacyBackslash chars) []
  ...
  do
    ...
    case input of
      Nothing ->
        return Skip
      Just chars ->
        let lines = addLine (stripLegacyBackslash chars) previousLines
         in case categorize lines of
              Done input -> return input
              Continue p -> readMore lines p

-- For compatibility with 0.19.0 such that readers of "Programming Canopy" by @jfairbank
