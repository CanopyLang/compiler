...
checkTagExistence git _pkg version = Task.io $ do
  exitCode <- _runGit git ["show", "--name-only", Version.toChars version, "--"]
  pure $ case exitCode of
    SysExit.ExitFailure _ -> Left (Exit.PublishMissingTag version)
...
checkForLocalChanges git hash version = Task.io $ do
  exitCode <- _runGit git ["diff-index", "--quiet", hash, "--"]
  pure $ case exitCode of
    SysExit.ExitSuccess -> Right ()
    SysExit.ExitFailure _ -> Left (Exit.PublishLocalChanges version)
