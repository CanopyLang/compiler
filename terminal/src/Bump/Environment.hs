...
--
-- @since 0.19.1
loadRegistry :: Http.Manager -> Task Exit.Bump Registry.ZokkaRegistries
loadRegistry manager = do
  zokkaCache <- Task.io Stuff.getZokkaCache
  reposConfig <- Task.io Stuff.getOrCreateZokkaCustomRepositoryConfig
  customRepos <- Task.eio BumpCustomRepositoryDataProblem (loadCustomRepositoriesData reposConfig)
  Task.eio Exit.BumpMustHaveLatestRegistry (Registry.latest manager customRepos zokkaCache reposConfig)
