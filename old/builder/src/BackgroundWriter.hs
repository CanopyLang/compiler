...
-- | Write JSON data to a file in a background thread within the given scope.
--
-- Similar to 'writeBinary', but uses JSON serialization instead of binary.
-- All writes are tracked and guaranteed to complete before the scope exits.
--
-- This function is used for writing interface files (.cani.json) and other
-- human-readable compiler artifacts.
--
-- ==== Examples
--
-- @
...
-- withScope $ \\scope ->
--   writeJSON scope "build/interfaces/Main.cani.json" moduleInterface
--
-- -- Write multiple interface files concurrently
-- withScope $ \\scope -> do
--   writeJSON scope "build/interfaces/Main.cani.json" mainInterface
--   writeJSON scope "build/interfaces/Utils.cani.json" utilsInterface
--   writeJSON scope "build/interfaces/Types.cani.json" typesInterface
-- @
--
-- @since 0.19.1
writeJSON :: (Aeson.ToJSON a) => Scope -> FilePath -> a -> IO ()
writeJSON (Scope workList) path value =
  do
    tvar <- STM.newTVarIO False
    _ <- Concurrent.forkIO (File.writeJSON path value >> STM.atomically (STM.writeTVar tvar True))
    STM.atomically $ STM.modifyTVar workList (tvar :)
