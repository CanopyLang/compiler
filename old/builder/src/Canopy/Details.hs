...
import qualified Canopy.Kernel.Discovery as KernelDiscovery
...
import qualified Bridge  -- NEW compiler integration
import qualified System.Environment as Env  -- For CANOPY_NEW_COMPILER check
...
loadInterfaces root (Details _ _ _ _ _ extras) = do
  case extras of
    ArtifactsFresh i _ -> do
      newTVarIO (Just i)
    ArtifactsCached -> do
      result <- File.readJSONOrRebuild (Stuff.interfaces root)
      let interfaces = either (const Nothing) Just result
            ...
            Right artifacts ->
              let objs = Map.foldr addObjects Opt.empty artifacts
               ...
               in do
                    ...
                    BW.writeJSON scope (Stuff.interfaces root) ifaces
    ...
    Right (Outline.Pkg (Outline.PkgOutline _ _ _ _ exposed deps _ _)) -> do
      ...
      case depResults of
        ...
        Right resolvedDeps -> do
          ...
          -- Check if NEW compiler should be used for dependencies
          useNewCompiler <- Bridge.shouldUseNewCompiler

          if useNewCompiler
            then do
              -- NEW COMPILER PATH: Use Bridge.compileFromExposed
              putStrLn ("NEW-COMPILER-DEBUG: Using NEW compiler for dependency package: " <> show pkg)
              buildWithNewCompiler pkg vsn exposed resolvedDeps cacheFilePath key f
            else do
              -- OLD COMPILER PATH: Use existing STM-based orchestration
              putStrLn ("OLD-COMPILER-DEBUG: Using OLD compiler for dependency package: " <> show pkg)
              buildWithOldCompiler pkg vsn exposed deps resolvedDeps cacheFilePath key f fs store manager zokkaRegistry
  where
    -- NEW compiler path for dependency builds
    buildWithNewCompiler pkg vsn exposed resolvedDeps cacheFilePath key f = do
      -- ELM-CSV DEBUG: Log dependency resolution results
      when (Pkg.toChars pkg == "BrianHicks/elm-csv") $ do
        putStrLn "ELM-CSV-DEBUG: *** DEPENDENCY RESOLUTION RESULTS (NEW COMPILER) ***"
      ...
      case sequenceA resolvedDeps of
        ...
        Right directArtifacts -> do
          -- Create Details for NEW compiler
          let exposedModules = case exposed of
                Outline.ExposedList mods -> mods
                Outline.ExposedDict dict -> concatMap snd dict

          case exposedModules of
            [] -> do
              Reporting.report key Reporting.DBroken
              return $ Left $ Just $ Exit.BD_BadBuild pkg vsn f
            (firstMod : restMods) -> do
              -- Load outline to create proper Details
              let exposedList = NE.List firstMod restMods

              -- Use Bridge.compileFromExposed (simplified - just report success for now)
              -- TODO: Actually compile with NEW compiler and extract artifacts
              putStrLn ("NEW-COMPILER-TODO: Would compile package " <> show pkg <> " with NEW compiler")
              putStrLn ("NEW-COMPILER-TODO: Exposed modules: " <> show exposedModules)

              -- For now, fall back to OLD compiler to avoid breaking the build
              -- This will be implemented in a follow-up
              putStrLn ("NEW-COMPILER-FALLBACK: Falling back to OLD compiler for now")
              buildWithOldCompiler pkg vsn exposed Map.empty resolvedDeps cacheFilePath key f Set.empty store undefined undefined

    -- OLD compiler path for dependency builds (existing code)
    buildWithOldCompiler pkg vsn exposed deps resolvedDeps cacheFilePath key f fs store manager zokkaRegistry = do
      -- ELM-CSV DEBUG: Log dependency resolution results
      when (Pkg.toChars pkg == "BrianHicks/elm-csv") $ do
  ...
  case Registry.lookupPackageRegistryKey zokkaRegistries pkg vsn of
    ...
    Just (Registry.PackageUrlKey packageData) ->
      do
        ...
        downloadPackageDirectlyToFilePath filePath (CustomRepositoriesData._url packageData) (CustomRepositoriesData._shaHash packageData) manager pkg
    Nothing ->
      let --FIXME
          blah = fmap show (Map.keys $ Registry._registries zokkaRegistries)
       in pure (Left $ Exit.PP_PackageNotInRegistry blah pkg vsn)

-- | Discover and store kernel modules after package extraction.
--
-- Scans the package directory for kernel modules and updates the global kernel registry.
-- The registry is stored in the Canopy home directory (~/.canopy/0.19.1/kernels.dat).
discoverAndStoreKernels :: Pkg.Name -> FilePath -> IO ()
discoverAndStoreKernels pkg packageDir = do
  -- Discover kernel modules from the package
  discoveries <- KernelDiscovery.discoverKernelModules pkg packageDir

  -- Only update registry if kernels were found
  when (not (null discoveries)) $ do
    -- Get global canopy home directory for registry storage
    canopyHome <- Stuff.getCanopyHome

    -- Read existing registry or create empty one
    maybeRegistry <- KernelDiscovery.readKernelRegistry canopyHome
    let registry = Maybe.fromMaybe KernelDiscovery.emptyRegistry maybeRegistry

    -- Insert discoveries for this package
    let updatedRegistry = KernelDiscovery.insertDiscoveries pkg discoveries registry

    -- Write updated registry back to disk
    KernelDiscovery.writeKernelRegistry canopyHome updatedRegistry

    putStrLn ("KERNEL_DISCOVERY: Found " <> show (length discoveries) <> " kernel modules in " <> show pkg)
...
downloadPackageDirectly cache packageUrl manager pkg vsn =
  let urlString = Utf8.toChars packageUrl
      packageDir = Stuff.package cache pkg vsn
   in Http.getArchiveWithFallback manager urlString Exit.PP_BadArchiveRequest (Exit.PP_BadArchiveContent urlString) $
        \(_, archive) -> do
          File.writePackage packageDir archive
          discoverAndStoreKernels pkg packageDir
          pure (Right ())

downloadPackageDirectlyToFilePath :: FilePath -> PackageUrl -> HumanReadableShaDigest -> Http.Manager -> Pkg.Name -> IO (Either Exit.PackageProblem ())
downloadPackageDirectlyToFilePath filePath packageUrl expectedShaDigest manager pkg =
  let urlString = Utf8.toChars packageUrl
   in Http.getArchiveWithFallback manager urlString Exit.PP_BadArchiveRequest (Exit.PP_BadArchiveContent urlString) $
        \(receivedShaHash, archive) ->
          if humanReadableShaDigestIsEqualToSha expectedShaDigest receivedShaHash
            then do
              ...
              discoverAndStoreKernels pkg filePath
              pure (Right ())
                ...
                Http.getArchiveWithHeadersAndFallback manager endpoint headers Exit.PP_BadArchiveRequest (Exit.PP_BadArchiveContent endpoint) $
                  \(sha, archive) ->
                    if expectedHash == Http.shaToChars sha
                      then do
                        let packageDir = Stuff.package cache pkg vsn
                        packageExists <- Dir.doesDirectoryExist packageDir
                        printLog (show packageExists <> ("C" <> packageDir))
                        File.writePackage packageDir archive
                        discoverAndStoreKernels pkg packageDir
                        pure (Right ())
